...
void SineFit(double freq, double timestep, UINT delay_ndx, 
		UINT capture_ndx, vector<double>& y, double* a)
{
	...
	yy = (-aa12*uu/aa11);
	zz = (-aa12*xx/aa22);
	a[1] = xx*bb1 + yy*bb2;		// sin part -- cos I think (Steven)
	a[0] = zz*bb1 + uu*bb2;		// cos part-- sin I think (Steven) 10/14/12
	return;
}

void cfft(std::vector<complex<double>>& data, UINT nn, int isign) {
	
	UINT i,j,m,n;
	UINT mmax,istep;
	complex<double> temp, w, wp;
	double theta, wtemp;
	n = nn;
...
//This is the Danielson-Lanczos section of the routine
	...
	while (n>mmax) {
		...
		theta = isign*(PI/mmax);
		wtemp = sin(0.5*theta);
		wp = complex<double>(-2.0*wtemp*wtemp, sin(theta));
...
/*  are equal to zero                                      */
   ...
   //AfxMessageBox("Problem with the values in the plot",MB_OK);
	   //I commented this out -- Steven, 10/14/12
   return 1;
   }
return 0;
} 
